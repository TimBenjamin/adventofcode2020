...
func prepare_input() {
	...
	for i, y := range input {
		for j, x := range y {
			if x == "#" {
				map_key = w + "," + z + "," + strconv.Itoa(i) + "," + strconv.Itoa(j)
...
func get_coords(coordinate string) []int {
	...
	w, _ := strconv.Atoi(coordinate_split[0])
	z, _ := strconv.Atoi(coordinate_split[1])
	y, _ := strconv.Atoi(coordinate_split[2])
	x, _ := strconv.Atoi(coordinate_split[3])
	return []int{w, z, y, x}
}
...
func get_coordinate(coords []int) string {
	...
	coord_strings = append(coord_strings, strconv.Itoa(coords[3]))
		...
		for z := -1; z <= 1; z++ {
			for y := -1; y <= 1; y++ {
				for x := -1; x <= 1; x++ {
					...
					coordinate := get_coordinate([]int{w + w_coord, z + z_coord, y + y_coord, x + x_coord})
		...
		for z := -1; z <= 1; z++ {
			for y := -1; y <= 1; y++ {
				for x := -1; x <= 1; x++ {
					...
					check_key := get_coordinate([]int{w + w_coord, z + z_coord, y + y_coord, x + x_coord})
